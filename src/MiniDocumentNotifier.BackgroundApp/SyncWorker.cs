...
namespace MiniDocumentNotifier.BackgroundApp
{
    public class SyncWorker
    {
        ...
        public void Run()
        {
            while (true)
            {
                var delaySeconds = RunOnce();
                var jitter = _random.NextDouble() * 0.2 - 0.1;
        ...
        public int RunOnce()
        {
            ...
            try
            {
                ...
                {
                    _logger.Info(
                        $"View configuration sync recovered after {_consecutiveFailures} consecutive failure(s); backoff reset.");
                }
            ...
            {
                _logger.Warning(
                    $"Applying backoff after {_consecutiveFailures} consecutive failure(s); waiting {delaySeconds}s before next sync attempt.");
            }
